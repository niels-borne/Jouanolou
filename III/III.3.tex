...
$$
\chi(E) = \rang (\Rd g_! E) = \sum_{i \in \mathbf{Z}} (-1)^i [\mathrm{H}^i_c (\overline{X}, \overline{E}):\mathbf{Q}_\ell],
$$
$$
\delta(E) = \det (\Rd g_! (E)) = \prod_{i \in \mathbf{Z}} (\det f_{\mathrm{H}^i_c(\overline{X}, \overline{E})})^{(-1)^i},
$$
...
\vskip .3cm
{
Proposition {\bf 3.6}. --- \it Soit $g: X \to \mathbf{F}_p$ un schéma projectif sur $\mathbf{F}_p$. On pose $K_X = \Rd g^! (\mathbf{Q}_\ell)$, et $\D_X = \Rd \underline{\Hom}_{\mathbf{Q}_\ell}(. , K_X)$. Alors, pour tout $E \in \D^b_c(X, \mathbf{Q}_\ell)$, on a l'identité
$$
L_{\D_X(E)}(t) = (-t)^{-\chi(E)} \delta(E) L_E(t^{-1}).
$$
}
\vskip .3cm
{\bf Preuve} : Le second membre a un sens d'après (3.3). Posons $S = \Spec$ et $\D_S = \Rd \underline{\Hom}_{\mathbf{Q}_\ell}(. , \mathbf{Q}_\ell)$. D'après (2.3.2 a)), on a 
$$
\Rd g_* (D_X E) \isomlong \D_S \Rd g_* (E),
$$
donc (3.3)
$$
L_{\D_X}(E) = L_{\D_S(\Rd g_* (E))}.
$$
Comme $L_E = L_{\Rd g_* (E)}$ (3.3), l'assertion résultera du lemme suivant
\vskip .3cm
{
...
\vskip .3cm
{
Proposition {\bf 3.9}. --- \it On suppose $X$ quasiprojectif, lisse et purement de dimension $n$ sur $\mathbf{F}_p$. Posant pour tout $E \in \D^b_c(S, \mathbf{Q}_\ell)$
$$
E^V = \underline{\Hom}_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell),
$$
...
\vskip .3cm
{\bf Preuve} : Comme $\D_X(E) = \Rd \underline{\Hom}_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(n))[2n]$, le cas (i) résulte du lemme suivant.
\vskip .3cm
{
Lemme {\bf 3.9.1}. --- \it Étant donnés un schéma noethérien $X$, $F \in \mathbf{Q}_\ell-\fsct(X)$ et $G \in \mathbf{Q}_\ell-\fscn(U)$, on a:
$$
\underline{\Ext}^j_{\mathbf{Q}_\ell}(F, G) = 0 \quad (j \geq 1).
$$
}
\vskip .3cm
Il s'agit de voir que si $F \in \mathbf{Z}_\ell-\fsct(X)$ et $G \in \mathbf{Z}_\ell-\fscn(X)$, les $\mathbf{Z}_\ell$-faisceaux $\Ext^j_{\mathbf{Z}_\ell}(F, G)$ $(j \geq 1)$ sont annulés par une puissance de $\ell$. D'après (I 6.4.2) et (II 1.2.1), on peut, quitte à se restreindre à des parties localement fermées convenables de $X$, supposer que $G \in \mathbf{Z}_\ell-\fsct(X)$. Alors, compte tenu de (II 1.26), l'assertion résulte de l'assertion analogue, bien connue, pour les $\mathbf{Z}_\ell$-modules de type fini. Montrons (ii). 

Il s'agit de voir que
$$
P^j = \underline{\Ext}^j_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(1)) = 0 \quad (j \geq 1).
$$
...
$$
\underline{\Ext}^j_{\mathbf{Q}_\ell}(E, \mathbf{Q}_\ell(1)) \times \underline{\mathrm{H}}^{2-j}_{\overline{x}}(E) \to \mathbf{Q}_{\ell'}
\leqno{(3.9.2)}
$$
avec (SGA5 I 4.5.1)
$$
\underline{\mathrm{H}}^{2-j}_{\overline{x}} = (\underline{\mathrm{H}}^{2-j}_x(E))_{\overline{x}}.
$$
...
$$
\underline{\mathrm{H}}^0_x(E) = \underline{\mathrm{H}}^1_x(E) = 0,
$$
d'où aussitôt le résultat annoncé.

Ceci dit, lorsque $X$ est projectif sur $\mathbf{F}_\ell$, la formule (3.6) prend la forme
$$
L_{E^\nu}(p^{-n} t) = (-1)^{- \chi (E)} \delta(E) L_E (t^{-1}),
\leqno{(3.10)}
$$
dans chacun des cas de (3.9). Compte tenu de (3.2 a)), cela résulte du lemme suivant.
\vskip .3cm
{
Lemme {\bf 3.11}. --- \it Soient $X$ un schéma de type fini sur $\mathbf{F}_\ell$, et $F \in \D^b_c(X, \mathbf{Q}_\ell)$. Posant $F(j) = F \otimes \mathbf{Q}_\ell (j)$ $(j \in \mathbf{Z})$, on a la relation
$$
L_{F(j)}(t) = L_F (p^{-j} t).
$$
}
...
$$
E^\nu \isomlong E(\rho) \quad \text{pour un}~\rho \in \mathbf{Z}.
$$
...
\begin{enumerate}
    \item[cas~(i)] $E \isomlong \mathbf{Q}_\ell(m)$ \quad pour un $m \in \mathbf{Z}$, et alors $p = -2m$.
    \item[cas~(ii)] $M \isomlong \mathbf{Q}_\ell(m)$ \quad pour un $m \in \mathbf{Z}$, et alors $p = -2m$.
\end{enumerate}
