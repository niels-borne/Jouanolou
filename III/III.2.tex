...
$$
\bRd f_! (f^* F \underline{\otimes}_{\mathbf{Z}_\ell}\mathbf{Z}_\ell(r)[2r]) \to F.
\leqno{(2.1.2)}
$$
Comme $A \underline{\otimes}_{\mathbf{Z}_\ell}\mathbf{Z}_\ell(r)$ est localement libre constructible, on définit sur le modèle de (1.1.7), mais sans hypothèse de degré sur $F$, un isomorphisme de ``projection''
$$
\bRd  f_! (\mathbf{Z}_\ell(r))[2r]\underline{\otimes}_{\mathbf{Z}_\ell} F \isomlong \bRd f_! (f^* F \underline{\otimes}_{\mathbf{Z}_\ell}\mathbf{Z}_\ell(r))[2r],
$$
...
$$
\bRd f_* \bRd \underline{\Hom}_A (E, \bRd f^! F) \to \bRd \underline{\Hom}_A(\bRd f_! E, F).
\leqno{(2.2.1)}
$$
Pour cela, nous allons d'abord définir, pour $L \in \D^-(X, A)$ et $M \in \D^+(X, A)$ un morphisme fonctoriel
$$
\bRd f_* \bRd \underline{\Hom}_A (L, M) \to \bRd \underline{\Hom}_A (\bRd f_! L, \bRd f_! M).
\leqno{(2.2.2)}
$$
On prendra alors pour (2.2.1) le morphisme composé
\[\begin{tikzcd}
	& {\bRd \underline{\Hom}_A(\bRd f_! E, \bRd f_! \bRd f^! F)} \\
	{\bRd f_* \bRd \underline{\Hom}_A(E, \bRd f^! F)} && {\bRd \underline{\Hom}_A(\bRd f_! E, F)}
	\arrow["{\bRd \underline{\Hom}_A (\id, \Tr_f)}", from=1-2, to=2-3]
...
$$
f_! \underline{\Hom}_A (E, F) \to \underline{\Hom}_A (f_! E, f_! F),
\leqno{(2.2.3)}
$$
à partir des morphismes analogues dans la catégorie des $A$-Modules. Pour définir (2.2.2) dans ce cas, on peut supposer $L$ quasilibre et $M$ flasque, de sorte que $\underline{\Hom}^{\bullet}_A(L, M)$ est flasque. Le morphisme (2.2.3) fournit par fonctorialité un morphisme de complexes
$$
f_! \underline{\Hom}^{\bullet}_A (L, M) \to \underline{\Hom}^{\bullet}_A (f_! L, f_! M).
\leqno{(2.2.4)}
$$
...
$$
\underline{\Hom}^{\bullet}_A(f_! L, f_! M) \to \underline{\Hom}^{\bullet}_A (P, Q).
$$
...
\leqno{(2.2.6)}
$$
à partir des morphismes analogues dans la catégorie des $A$-Modules. Pour définir (2.2.5), on peut supposer $L$ quasilibre et $M$ flasque, de sorte que $\underline{\Hom}^{\bullet}_A(L, M)$ est flasque. Le morphisme (2.2.6) fournit par fonctorialité un morphisme de complexes
$$
f_* \underline{\Hom}_A (L, M) \to \underline{\Hom}^{\bullet}_A (f_* L, f_* M).
\leqno{(2.2.7)}
$$
On prend pour (2.2.5) le composé de (2.2.7) et du morphisme
$$
\underline{\Hom}^{\bullet}_A (f_* L, f_* M) \to \underline{\Hom}^{\bullet}_A (P, f_* M)
$$
...
\begin{itemize}
    \item[(i)] $\bRd f_* \bRd \underline{\Hom}_A (E, \bRd f^! F) \isomlong \bRd \underline{\Hom}_A (\bRd f_! E, F)$.
...
$$
\bRd f_* \bRd \underline{\Hom}_{A_m}(E_m, \bRd f^! F_n) \to \bRd \underline{\Hom}_{A_m}(\bRd f_! E_m, F_n) \quad (m, n \in \mathbf{N}; m \geq n)
$$
...
$$
\D_K(F) = \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(F, K).
$$
\vskip .3cm
{
...
$$
\Hom_{\mathbf{Z}_\ell}(\D_K (F) \underline{\otimes} F, K) \to \Hom_{\mathbf{Z}_\ell}(\D_K(F), \D_K(F))
$$
est un isomorphisme (II 2.6). L'image inverse de l'identité de $\D_K(F)$ définit un morphisme
$$
F \underline{\otimes} \D_K(F) \to K.
$$
...
\vskip .3cm
{\bf Preuve} : Montrons par exemple (ii), les autres assertions se prouvant de fa\c{c}on essentiellement identique, à partir des énoncés correspondants de (SGA5 I). Montrons tout d'abord que si $E \in \D^b_c(T, \mathbf{Z}_\ell)$, alors $\bRd \underline{\Hom}_{\mathbf{Z}_\ell}(E, \bRd f^!(\mathbf{Z}_\ell)) \in \D^b_c(T, \mathbf{Z}_\ell)$.
\vskip .3cm
{
...
$$
\bRd \underline{\Hom}_{\mathbf{Z}_\ell}(E, \bRd f^! (\mathbf{Z}_\ell)) \in \D^+_c(T, \mathbf{Z}_\ell).
$$
...
\begin{itemize}
    ...
    $$
    p^* K_X \underline{\otimes} q^* K_Y \isomlong K_{X \times_S Y}.
    $$
    \item[b)] Pour $F \in \D^b_c(X, \mathbf{Z}_\ell)$ et $G \in \D^b_c(X, \mathbf{Z}_\ell)$, il existe un isomorphisme canonique fonctoriel
    $$
    p^* \D_X(F) \underline{\otimes} q^* \D_Y(G) \isomlong D_{X \times_S Y}(p^* F \underline{\otimes}q^* G).
    $$
\end{itemize}
}
\vskip .3cm
...
$$
\underline{\Ext}^{2d-i}_{\mathbf{Q}_\ell}(F, \mathbf{Q}_\ell(d))_x \times \mathrm{H}^i_x(F) \to \mathbf{Q}_\ell.
$$
}
\vskip .3cm
...
$$
\D_X(F)_x \isomlong \bRd \Hom_{\mathbf{Q}_\ell}(\bRd \Gamma_x(F), \mathbf{Q}_\ell).
$$
...
\begin{itemize}
    ...
    $$
    \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(p^* F, q^* G) \isomlong p^* \D_X(F) \underline{\otimes} q^* G.
    $$
    \item[b)] Il existe un accouplement parfait canonique
    $$
    \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(p^* F, \bRd q^! G) \times \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(q^* G, \bRd p^! F) \to K_{X \times_S Y}.
    $$
\end{itemize}
}
\vskip .3cm
...
\begin{itemize}
    \item[(i)] $\bRd h_* \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(p^* F, \bRd q^! G) \isomlong \bRd \underline{\Hom}_{\mathbf{Z}_\ell}(\bRd p_! (F), \bRd q_* (G))$. 
...
\[\begin{tikzcd}
	& {\mathrm{H}^0_\phi(X \times_S Y, \bRd \underline{\Hom}(p^* F, \bRd q^* G)) \times \mathrm{H}^0_\psi(X \times_S Y, \bRd \underline{\Hom}(q^* G, \bRd p^! F))} \\
