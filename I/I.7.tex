...
$$
\underline{\otimes}: \D^-(X, A) \times \D^-(X, A) \to \D^-(X, A),
\leqno{(7.2.1)}
$$
...
\begin{itemize}
    ...
    $$
    \mathrm{H}^i(K \underline{\otimes} E) = 0 \quad \text{pour} \quad i \notin [m, n]
    $$
...
$$
\underline{\otimes}: \D^-(X, A)_{\torf} \times \D (X, A) \to \D(X, A)
\leqno{(7.2.3)}
$$
...
\end{tikzcd}\leqno{\underline{\otimes}}\]
...
\vskip .3cm
{\bf 7.2.5}. Soient $K$ et $L$ deux complexes de $A$-faisceaux sur $X$. Dans chacun des cas où nous avons défini $K \otimes L$, nous poserons
$$
\cTor^A_i(K, L) = \mathrm{H}^{-i}(K \underline{\otimes} L) \quad (i \in \mathbf{Z}).
$$
...
$$
(\overline{\Hom}^\bullet_A (K, L))^n = \bigoplus_{p \in \mathbf{Z}} \overline{\Hom}_A (\id_K, d_L).
$$
...
\vskip .3cm
{\bf Preuve} : D'après (2.8), le complexe $\Hom^\bullet_A (K, L)$ est obtenu en appliquant le foncteur $\varinjlim_{n \in \mathbf{N}}$ au complexe $\overline{\Hom}^\bullet_A(K, L)$. Or ce dernier est acyclique (7.4.2) et l'hypothèse sur $L$ implique que ses composants sont directement stricts. On conclut grâce à (EGA $0_{III}$ 13.2.2). 
...
$$
pour uniformiser les notations. La preuve de (i), semblable à celle de (7.3.5), est laissé au lecteur. Montrons (ii) dans le cas $i = 0$, le cas général s'en déduisant aussitôt. On peut supposer $K$ quasilibre et $L$ à composants flasques et directement stricts. Alors, notant $\Hom_{ht}(K, L)$ les morphismes de $K$ dans $L$ dans la catégories $\K(X, A)$, on est ramené à prouver le corollaire suivant : 
\vskip .3cm
{
...
\[\begin{tikzcd}
	...
	\arrow["{d^\bullet_1}"', dashed, from=1-2, to=2-1]
...
$$
\bRd \cHom_A(K \underline{\otimes} L, M) \to \bRd \cHom_A(K, \bRd \cHom_A(L,M)), 
\leqno{(7.6.2)}
$$
...
$$
\bRd \overline{\Hom}_A(K \underline{\otimes}L, M) \to \Rd \overline{\Hom}_A(K, \bRd \cHom_A(L, M)).
\leqno{(7.6.3)}
$$
$$
\bRd \Hom_A(K \underline{\otimes}L, M) \to \Rd \Hom_A(K, \bRd \cHom_A(L, M)).
\leqno{(7.6.4)}
$$
$$
\Hom_A(K \underline{\otimes}L, M) \to \Hom_A(K, \bRd \cHom_A(L, M)).
\leqno{(7.6.5)}
$$
...
\vskip .3cm
{\bf Preuve} : Soit $u: K \underline{\otimes} L \to M$. On obtient un morphisme $K \to \bRd \cHom_A (L, M)$ en composant $\bRd \cHom_A(\id_L, u)$ avec (7.6.7).
\vskip .3cm
...
$$
\bRd \cHom_A(E, F) \underline{\otimes}G \to \bRd \cHom_A(E, F \underline{\otimes} G).
\leqno{(7.6.9.2)}
$$
...
\begin{itemize}
    ...
    $$
    f^* (E) \underline{\otimes}_A f^* (F) \isomlong f^*(E \underline{\otimes}_A F),
    $$
...
\begin{itemize}
    ...
    $$
    \bRd i_! (E \underline{\otimes} i^* (F)) \isomlong \bRd i_! (E) \underline{\otimes} F
    $$
...
\begin{itemize}
    ...
    $$
    \bRd j_* (E \underline{\otimes} \bRd j^* (E)) \isomlong \bRd j_* (F) \underline{\otimes} E
    $$
    dans chacun des cas suivants :
    \begin{itemize}
