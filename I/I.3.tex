...
\vskip .3cm
Si $F$ est de type strict, nous avons vu dans l courant de la démonstration que pour tout entier $n \geq 0$, le système projectif 
$$
...
$$
d'où, en composant avec a projection canonique, un foncteur additif
$$
...
$$
avec les morphismes de transition évidents. De plus, on a, de fa\c{c}on évident un $\mathcal{E}(X, J)$-morphisme fonctoriel en $F$
$$
u_F : \overline{F} \to F.
$$
...
$$
et $F$ par $\overline{F}'' \times_{F''} F$, on peut de plus supposer que $F''$ est aussi essentiellement constant. Dans ce cas, $F$ l'est également, d'où l'assertion.
\vskip .3cm
{
Corollaire {\bf 3.7}. --- \it La sous-catégorie pleine $\TC(X, J)^+$ de $A-\fsc(X)$ engendrée par les $A$-faisceaux isomorphes à un $A$-faisceau de la forme $\mathcal{E}_X(M)$, avec un $A_X$--Module annulé par une puissance de $J$, est \emph{exacte}.
}
\vskip .3cm
...
v_i: E|T_i \to F|T_i
$$
vérifiant $\varphi_{T_i}(v_i) = a|T_i$. D'après l'injectivité es applications $\varphi_{T_i \times T_j}$, les morphismes $v_i$ se recollent en un morphisme $v$ de $\mathcal{E}(X, J)$ vérifiant $\varphi_X(v) = a$ localement, donc aussi globalement (1.7.2).
\vskip .3cm
{
...
\begin{itemize}
    ...
    $$
    U_{ij} = U_i \times U_k \quad \text{(resp.}~U_{ijk} = U_i \times U_j \times U_k).
    $$
\end{itemize}
...
$$
\theta_j \circ \theta_i = \theta_i \quad \text{sur} \quad U_{ij}.
$$
}
\vskip .3cm
