...
\vskip .3cm
{\bf 6.4.8}. Soient $X$ un topos, $U$ un ouvert de $X$ et $Y$ le topos ferm\'e complémentaire. On note $j: Y \to X$ le morphisme de topos canonique~: on rappelle (SGA4 \quad) que $j$ est \emph{cohérent}. Étant donnés deux $A$-faisceaux $E$ et $F$ sur $Y$ respectivement, on définit comme précédemment un \emph{isomorphisme} de bifoncteurs
$$
...
h_\gamma(P) \to F \to 0,
$$
où $(h_\gamma (P))_p = P_{\gamma(p)}$ pour tout $p \geq 0$. Localement, la section de $F_n$ est l'image d'une section de $P_{\gamma(n)}$, donc d'une section de $P_{\gamma(n+r)}$ pour tout entier $r \geq 0$ ($h_\gamma(P)$ est \emph{directement} strict). Le lemme en résulte aussitôt.
