...
On suppose sonné dans ce paragraphe un idéotope $(X, A, J)$, et on convient de poser pour tout entier $n \geq 0$
$$
A_n = A/J^{n+1}.
$$
\vskip .3cm
...
\begin{itemize}
    ...
    \item[(b)] Pour tout suite exacte $0 \to E' \to E \to E'' \to 0$ (resp. $0 \to F' \to F \to F'' \to 0$) de $\mathcal{E}$, le foncteur $\pi$ transforme les morphismes bords
    $$
...
$$
\cTor_i(M, G) = 0 \quad (i > \text{dp}_A(M)).
$$
}
\vskip .3cm
...
f^*: A-\Mod_Y \to A-\Mod_X.
$$
On rappelle qu'on en déduit pour tout couple $(P, Q)$ de $A_{nY}$--Modules ($i$ entier $\geq 0$) et tout entier $i \geq 0$ un isomorphisme fonctoriel
$$
